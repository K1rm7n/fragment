<template>
  <div class="relative">
    <div class="lg:grid grid-cols-2 relative z-10">
      ...
      >
        <div class="space-y-2">
          <h2 class="text-3xl font-bold mb-5 text-dark">Model Viewer</h2>
          <p>Easily display interactive 3D models on the web & in AR</p>
          <p>
            Path:
            <code>/components/example/</code> in
            <code>ModelViewer.client.vue</code>
          </p>
          <p>
            The component must necessarily contain <code>.client</code> in its
            name because <code>@google/model-viewer</code> must be rendered
            exclusively on the client side to prevent errors.
          </p>
          <a
      ...
      <div class="relative clip-block-corner">
        <div class="z-10 bg-alt/[0.45] relative backdrop-blur-3xl h-full">
          <div class="h-full md:min-h-[500px]">
            <div class="h-full flex justify-center">
              <model-viewer
                ...
                camera-controls
                touch-action="pan-y"
                class="w-full h-full max-sm:min-h-[100vw] max-lg:min-h-[500px]"
                disable-zoom
              ></model-viewer>
            </div>
          </div>
