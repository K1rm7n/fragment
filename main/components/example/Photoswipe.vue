    ...
    <div class="lg:grid grid-cols-2 relative z-10">
      ...
      >
        <div class="space-y-2">
          ...
          <p>Best gallery for your website</p>
          <p>
            Path:
            <code>/components/example/Photoswipe.vue</code>
          </p>
          <p>
            Everything you need to create the perfect gallery with great
            documentation and no third-party dependencies.
          </p>
          <p>
            On the
            <code>/plugins/kinesis.js</code> path you will find everything about
            registering a Photoswipe in the application.
          </p>
          <p>
            Photoswipe needs to be rendered on the client side, so everything
            associated with it should be wrapped in the following construction
            <code class="whitespace-nowrap">onMounted(() => {...})</code>
          </p>
          <a
            href="https://photoswipe.com/styling/"
            target="_blank"
            ...
            <div class="h-full flex justify-center">
              <div class="gallery grid grid-cols-3">
                <a
                  v-for="index in 9"
                  :key="index"
                  :href="'https://picsum.photos/2000/2000?n' + index"
                  :data-pswp-width="2000"
                  :data-pswp-height="2000"
                  target="_blank"
                  rel="noreferrer"
                >
                  <img :src="'https://picsum.photos/2000/2000?n' + index" />
                </a>
              </div>
            </div>
          </div>
...
<script setup>
const { $PhotoSwipeLightbox: Lightbox, $PhotoSwipe: photoswipe } = useNuxtApp()
onMounted(function () {
  const gallery = new Lightbox({
    closeSVG:
